...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-screen h-auto flex-col items-center justify-center overflow-hidden bg-neutral-950 w-full z-0 pb-16",
        className
      )}>
      <div className="relative mt-56 flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0 ">
        <motion.div
        ...
        <motion.div
          ...
          className="absolute inset-auto z-50 h-0.5 w-[20rem] -translate-y-[7rem] bg-cyan-400 "></motion.div>

        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-neutral-950 "></div>
      </div>

      <div className="relative z-50 -mt-16 flex flex-col items-center px-5">
        {children}
      </div>
    </div>
  );
